...
def discrim_conv(batch_input, out_channels, stride, k_size):
    # padding for each side (no padding for batch_size, channel)
...
def create_model(inputs, targets, segments, labels, label_weights, bbox_targets, bbox_loss_weights):
    ...
    with tf.variable_scope("generator"):
        ...
        # 512x512 -> 512x640 (for fitting to original size)
    ...
    with tf.name_scope("generator_loss"):
        ...
        # pass segmentation loss to generator for making it better images to detect pedestrian
